 ...
 *
 * Usage:
 *   npx tsx init-agents.ts [--refactor]
 *
 ...
 *                AGENTS.md.bak.
 *
 * Out of scope: monorepos with nested per-package AGENTS.md. This tool
 * assumes a single-root setup.
 */
...
function scaffoldRefactor(root: string): void {
	...
	for (const rawLine of original.split('\n')) {
		...
		const isEssential =
			...
			/\bbuild\b/.test(lower) && /\b(pnpm|yarn|npm|bun|corepack|make|cargo|go|gradle|mvn)\b/.test(lower) ||
	...
	for (const domain of DOC_DOMAINS) {
		...
		logFinding({ kind: 'create', message: `Wrote docs/${domain}.md (${buckets[domain].length} imported lines)` })
	}
	...
	console.log('2. Resolve any contradictions surfaced by the refactor (see references/refactor.md).')
...
function buildRefactoredRoot(essentials: string[], unclassified: string[]): string {
	...
	// First entry is the original "# Heading"; the description (if captured) is the
	// first non-heading prose line that followed it. If no description was captured,
	// fall back to the original heading text without the "#" prefix.
	const headingIdx = essentials.findIndex((l) => l.startsWith('# ') && !l.startsWith('# AGENTS'))
	const headingText =
		headingIdx >= 0 ? essentials[headingIdx].replace(/^#\s+/, '').trim() : ''
	const descriptionIdx = essentials.findIndex(
	...
	if (unclassified.length > 0) {
		lines.push('')
		lines.push('<!-- TODO: The following lines could not be auto-classified. Move to a docs/ file or delete. -->')
